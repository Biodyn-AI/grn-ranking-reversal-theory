...
\textsc{random}); reference shifts on the immune slice with three reference
networks (\textsc{hpn\_dream}, \textsc{beeline\_gsd}~\cite{pratapa2020},
and a \textsc{dorothea}~\cite{holland2020}\,/\,\textsc{trrust}~\cite{han2018}
union); mapping shifts on four policies of a probe-prior subset (also
spanning the \textsc{omnipath}~\cite{turei2021} reference).
...
applies; \emph{(c)}~at least two metrics per rank claim (AUPR + AUROC);
\emph{(d)}~retain the calibration-vs-base-rate magnitude ratio.

\paragraph{Reproducibility.}
\url{https://anonymous.4open.science/r/grn-ranking-reversal-theory-DB8E/}

\renewcommand{\bibfont}{\fontsize{5.8}{6.6}\selectfont}
...
\emph{J.~Human Resources} 50:317--372, 2015.
